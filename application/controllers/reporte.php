...
//_____________________________________________________
    public function parametros()
    {
       ...
       #if(isset($id) && $id !="")
       {
           ...
           $parametros["controlador"] = "reporte";
           $parametros["titulo_general"] = "Reportes";
           $parametros["funcion"] = "";
           $parametros["carpeta"] = "reporte";
...
public function index()
{
    ...
    $datos = $parametros;
    $datos["Salida"] = $this->reporte_model->indexreportes();
    $this->load->view($parametros["estilo"],$datos);
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
    $this->load->view("footer");
}
//_____________________________________________________
public function vista($targeta_id)
{
    $datos = array();
    $parametros = $this->parametros();
    $parametros["funcion"] = "vista";
    $datos = $parametros;
	$datos["Tarjeta"] = $this->targeta_model->consulto($targeta_id);
    $datos["Salida"] = $this->reporte_model->consulto($targeta_id);
    $this->load->view($parametros["estilo"], $datos);
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
    $this->load->view("footer");
}
//_____________________________________________________
public function clonar($targeta_id)
{
	$parametros = $this->parametros();
	$reporte = $this->reporte_model->nuevo($targeta_id);
	redirect($parametros["controlador"]."/vista/".$targeta_id);
}
//_____________________________________________________
public function add()
{
    ...
    if (isset($_POST["post"]) )
    {
        #if ($_POST['opcion']!="")
        {
            ...
            $resultado = $this->reporte_model->nuevo($_POST);
            if($resultado != "") 
            {
...
public function edit($id)
{
    ...
    $datos = $parametros;
    $datos["Salida"] = $this->reporte_model->consulto($id);
    if (isset($_POST["post"]) )
    {
        #if ($_POST['opcion']!="")
        {
            unset($_POST["post"]);
            #echo "<pre>"; print_r($_POST); echo "</pre>"; exit;
            $resultado = $this->creador_model->actualizo($_POST);
            if($resultado == 1) 
            {
...
//_____________________________________________________
public function buscar($inicio, $fin)
{
    $datos = array();
    $parametros = $this->parametros();
    $parametros["funcion"] = "buscar";
    $datos = $parametros;
	$datos["Salida"]  = $this->hacer_model->consultoRango($inicio, $fin);
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
}
//_____________________________________________________
}
